 ...
 * 
 */
public class Query2<T> extends QueryCondition<T>
        implements
            QueryExecuteI<T>,
            QueryOtherI<Query2> {
	...
	public Query2(SQLManager sqlManager, Class<T> clazz) {
	 ...
	 * @return
	 */
	public Query2<T> condition() {
		return new Query2(this.sqlManager, clazz);
	}

	public LamdbaQuery<T> lambda() {
		if (BeanKit.queryLambdasSupport) {
			LamdbaQuery newQuery = new LamdbaQuery(this.sqlManager, clazz);
			if (this.sql != null || this.groupBy != null
			        || this.orderBy != null) {
				throw new UnsupportedOperationException(
				        "LamdbaQuery必须在调用其他AP前获取");
			}
			return newQuery;
		} else {
			throw new UnsupportedOperationException(
			        "需要使用Java8以上，并且依赖com.trigersoft:jaque,请查阅官网文档");
		}
	...
	public List<T> select(String... columns) {
		...
		sb.append(" FROM ").append(getTableName(clazz)).append(" ")
		        .append(getSql());
		this.setSql(sb);
		makeSql();
	...
	public Map mapSingle() {
		List<Map> list = limit(getFirtRowNumber(), 1).selectByType(Map.class);
		if (list.isEmpty()) {
			return null;
		}
		// 同SQLManager.single 一致，只取第一条
	...
	private List selectByType(Class retType) {
		StringBuilder sb = new StringBuilder("SELECT * ");
		sb.append("FROM ").append(getTableName(clazz)).append(" ")
		        .append(getSql());
		this.setSql(sb);
	...
	private void makeSql() {
		...
		if (this.startRow != -1) {
			setSql(new StringBuilder(
			        sqlManager.getDbStyle().getPageSQLStatement(
			                this.getSql().toString(), startRow, pageSize)));
		}
	}

	@Override
	public int update(Object t) {
		SQLSource sqlSource = this.sqlManager.getDbStyle()
		        .genUpdateAbsolute(clazz);
	...
	private int handlerUpdateSql(Object t, SQLSource sqlSource) {
		if (this.sql == null || this.sql.length() == 0) {
			throw new BeetlSQLException(BeetlSQLException.QUERY_CONDITION_ERROR,
			        "update操作没有输入过滤条件会导致更新所有记录");
		}

		GroupTemplate gt = this.sqlManager.getBeetl().getGroupTemplate();
		Template template = gt.getTemplate(sqlSource.getTemplate(),
		        this.tempLoader);
		...
		List<SQLParameter> param = (List<SQLParameter>) template.getCtx()
		        .getGlobal("_paras");
	...
	@Override
	public Query2<T> having(QueryCondition condition) {
		// 去除叠加条件中的WHERE
	...
	@Override
	public Query2<T> groupBy(String column) {
	...
	@Override
	public Query2<T> orderBy(String orderBy) {
	...
	@Override
	public Query2<T> asc(String column) {
		OrderBy orderByInfo = this.getOrderBy();
	...
	@Override
	public Query2<T> desc(String column) {
		OrderBy orderByInfo = this.getOrderBy();
	 ...
	 */
	@Override
	public Query2<T> limit(long startRow, long pageSize) {
