 ...
 * @date 2017/11/5
 */
public class Java6Query<T> extends QueryCondition<T> implements QueryExecuteI<T>, QueryOtherI<Java6Query> {

    Class<T> clazz = null;

    public Java6Query(SQLManager sqlManager, Class<T> clazz) {
     ...
     * @return
     */
    public Java6Query<T> condition() {
        return new Java6Query(this.sqlManager, clazz);
    }
    ...
    @Override
    public Java6Query<T> having(QueryCondition condition) {
        //去除叠加条件中的WHERE
    ...
    @Override
    public Java6Query<T> groupBy(String column) {
    ...
    @Override
    public Java6Query<T> orderBy(String orderBy) {
    ...
    @Override
    public Java6Query<T> limit(long startRow, long pageSize) {
