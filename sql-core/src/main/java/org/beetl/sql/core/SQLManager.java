...
import org.beetl.sql.core.call.CallParser;
...
public class SQLManager implements DataAPI {
	...
	public CallResult callUpdate(String call,Map paras,Class clazzs[]){
		return call(call,paras,clazzs,false);
	}

	public CallResult callQuery(String call,Map paras,Class clazzs[]){
		return call(call,paras,clazzs,true);
	}

	protected CallResult call(String call,Map paras,Class[] clazz,boolean isQuery){
		CallParser callParser = new CallParser(call);
		callParser.parse();
		String jdbcCall = callParser.getJdbcSQL();
		List<CallParam>  params = callParser.getCallParam();

		SqlId id = this.sqlIdFactory.buildSql(jdbcCall);
		SQLSource source = new SQLSource(id, jdbcCall);
		source.setSqlType(SQLType.CALL);
		ExecuteContext executeContext = ExecuteContext.instance(this).initSQLSource(source);
		SQLExecutor script = dbStyle.buildExecutor(executeContext);

		CallResult callResult = script.call(jdbcCall,paras,params,clazz,isQuery);
		return callResult;
