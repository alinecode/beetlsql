...
* 社区 http://ibeetl.com
* qq群 219324263
* 当前版本 2.4.1 (190K), 另外还需要beetl(http://git.oschina.net/xiandafu/beetl2.0/attach_files) 包
**********************************************************************
...
//使用内置的生成的sql 新增用户，如果需要获取主键，可以使用insert(user,true);
User user = new User();
user.setAge(19);
user.setName("xiandafu");
sqlManager.insert(user);

//使用内置sql查询用户,unique 按照主键查询，如果没有找到，会抛异常，有可以使用single((User.class,id))
int id = 1;
