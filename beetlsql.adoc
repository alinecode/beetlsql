...
BeetlSQL 提供一个PageQUery对象,用于web应用的翻页查询,此时sqlId 要求使用page函数或者pageTag标签,这样才能同时获得查询结果集总数和当前查询的结果
...
如果字段较多,为了输出方便,也可以使用pageTag 标签函数,如果字段简单,可以用page函数,具体参考pageTag和page函数说明

    PageQuery query = new PageQuery(); //从第一页开始查询,无参数
